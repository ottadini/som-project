# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#
# Thermal conductivity + well-log data: k-means clustering (103 observations)
#
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
# can be produced by the following function 'wssplot'.
...
wrap <- function(string, w=0.9 * getOption("width")){
    # Returns a string wrapped by "\n" to width w
    paste(strwrap(string, width=w), collapse="\n")
}

# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# 1. Load and standardise data
...
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# 2. Determine number of clusters using iterations of k-means:
wssplot(df, nc=10)
...
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# 3. NbClust package to test ideal cluster number.
library(NbClust)
...
par(mfrow=c(1, 1))
caption <- "Tally of number of clusters chosen by 30 criteria"
wraplen <- 0.6 * nchar(title)
barplot(table(nc$Best.n[1, ]), 
        xlab="Numer of Clusters", ylab="Tally of Criteria",
        main=wrap(caption, w=wraplen))
